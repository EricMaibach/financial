...
Collects daily financial information on credit markets:
- High-yield credit spread (FRED: BAMLH0A0HYM2)
- Investment Grade Credit Spread (FRED: BAMLC0A0CM)
- Leveraged Loan Proxy (BKLN ETF)
- Credit ETF spreads vs treasuries (LQD, HYG vs IEF)
- CCC-rated bond spread (FRED: BAMLH0A3HYC)
...
class CreditSignalsTracker:
    """Tracks credit market signals and stores them in CSV files."""
    ...
    def __init__(self, data_dir="data", fred_api_key=None):
        ...
        # ETF tickers for tracking
        self.etf_tickers = {
            'leveraged_loan': 'BKLN',  # Invesco Senior Loan ETF
            'high_yield_credit': 'HYG',  # iShares iBoxx High Yield Corporate Bond ETF
            'investment_grade_credit': 'LQD',  # iShares iBoxx Investment Grade Corporate Bond ETF
            'treasury_7_10yr': 'IEF'  # iShares 7-10 Year Treasury Bond ETF
        }
    ...
    def fetch_etf_data(self, ticker, start_date=None, end_date=None):
        ...
        try:
            ...
            if start_date:
                ...
                df = etf.history(period="5d")  # Get last 5 days by default
    ...
    def calculate_etf_spreads(self, credit_etf_data, treasury_etf_data):
        ...
        This is a simplified proxy using price ratios.

        Args:
            credit_etf_data: DataFrame with credit ETF prices
            treasury_etf_data: DataFrame with treasury ETF prices
    ...
    def append_to_csv(self, df, filepath, date_column='date'):
        ...
        if filepath.exists():
            ...
            # Find dates that don't exist in the file
    ...
    def collect_etf_signals(self, lookback_days=12775):
        ...
        for signal_name, ticker in self.etf_tickers.items():
            ...
            filepath = self.data_dir / f"{signal_name}_etf.csv"
            last_date = self.get_last_date_in_file(filepath)
        ...
        # Calculate credit ETF spreads vs treasuries
        print("\n=== Calculating Credit ETF Spreads ===")

        # Get treasury data for spread calculation
        treasury_file = self.data_dir / "treasury_7_10yr_etf.csv"
        if treasury_file.exists():
            treasury_df = pd.read_csv(treasury_file)
            treasury_df.columns = ['date', 'close']

            # Calculate HYG vs IEF spread
            hyg_file = self.data_dir / "high_yield_credit_etf.csv"
            if hyg_file.exists():
            ...
            # Calculate LQD vs IEF spread
            lqd_file = self.data_dir / "investment_grade_credit_etf.csv"
            if lqd_file.exists():
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Collect credit market signals and store them for historical tracking.'
    )
    parser.add_argument(
        '--data-dir',
        default='data',
